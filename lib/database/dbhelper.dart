...
class DBHelper {
  ...
  setDB() async {
    ...
    String path = join(directory.path, 'cashbook.db');
  ...
  void _onCreate(Database db, int version) async {
    ...
    await db.rawInsert(
      "INSERT INTO user (name, username, password) VALUES ('Erina Seviyanti', 'erina', 'password')",
    );
    print('DB Created');
  }
  ...
  Future<String> getPassword() async {
    ...
    List<Map> list = await dbClient!.rawQuery(
      "SELECT password FROM user WHERE username = 'erina'",
    );
  ...
  Future<bool> updatePassword(String password) async {
    ...
    int res = await dbClient!.rawUpdate(
      "UPDATE user SET password = '$password' WHERE username = 'erina'",
    );
